...
int main(int argc, char *argv[]){
    ...
    //NetDeviceContainer p1e1Connect;
    //p1e1Connect = pointToPoint.Install(p1e1);
    //NetDeviceContainer p2e2Connect;
    //p2e2Connect = pointToPoint.Install(p2e2);
    NetDeviceContainer e1a1Connect;
    e1a1Connect = pointToPoint.Install(e1a1);
    NetDeviceContainer e2a2Connect;
    e2a2Connect = pointToPoint.Install(e2a2);
    NetDeviceContainer a1c1Connect;
    a1c1Connect = pointToPoint.Install(a1c1);
    NetDeviceContainer a2c1Connect;
    a2c1Connect = pointToPoint.Install(a2c1);
    ...
    ipp1e1 = address.Assign(p1e1ChannelConnect);

    address.SetBase ("10.1.2.0", "255.255.255.0");
    Ipv4InterfaceContainer ipp2e2;
    ipp2e2 = address.Assign(p2e2ChannelConnect);
    ...
    ApplicationContainer apps = server.Install (p1.Get (0));
    apps.Start (Seconds (1.0));
    apps.Stop (Seconds (10.0));

    UdpEchoClientHelper client (ipp1e1.GetAddress (0), 9);
    client.SetAttribute ("MaxPackets", UintegerValue (10));
    client.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
    client.SetAttribute ("PacketSize", UintegerValue (1024));

    apps = client.Install (NodeContainer (p2.Get (0)));
    ...
    // UdpEchoClientHelper echoClient (ipp2e2.GetAddress (0), 9);
